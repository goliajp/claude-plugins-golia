...
- **0.4.3** — write handoff.md in the user's session language. Hardcoded English structural tokens (`**Mode A — path defined.**`, `Pickup signal:`, `### Open question` / `### Candidates` / `### What we know` / `### What we don't know`, `Option N`, `Known constraint`, etc.) used to leak into Chinese-session handoffs, mixing badly with the free-text Chinese prose. New `## Language consistency` rule: hardcoded English in the templates is **schema, not literal** — preserve the structure, translate the wording per session language (signals: CLAUDE.md directive, recent user turns). Metadata line gains a typed `mode: A` / `mode: B` field, which resume reads as the language-agnostic anchor instead of grepping the translated Mode label text. Mode A / Mode B / Pickup signal rules switched from "literally" to "schema (English); translate per session", with Chinese example phrasings inline (`**模式 A — 路径已定。**`, `继续指示：...`). Resume step 2 + step 4 now source mode from metadata. Backwards compatible — old handoffs without `mode:` still print verbatim.
